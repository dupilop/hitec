...
?>
        ...
        return false;
              ...
              <div class="table-responsive display">
                ...
                $asd = $pdo->query("SELECT * FROM masiksavings ms
                  ...
                  WHERE s.s_a_id='$userid' || c.c_created_by='$userid' GROUP BY ms.ms_dateupload ORDER BY ms_dateupload DESC 
                  ");
                ...
                <table class="table table-bordered display table-hover table-sm" width="100%" cellspacing="0" id="example"  style="width:100%">
                  ...
                  <tbody>
                    ...
                    echo '<tr>';
                      ...
                      echo '<td>'.$year.'</td>';
                        echo '<td>'.$date.'</td>';
                        echo '<td><button class="btn btn-danger remsav" id="'.$a['ms_id'].'" data-toggle="tooltip" data-placement="top" title="Remove Saving"><i class="fa fa-times" aria-hidden="true"></i></button></td>';
                        echo '</tr>';
  ...
  <script type="text/javascript">
    $(document).on("click", ".remsav", function(e){
      var id = $(this).attr('id');
      var data = 'ms_id=' + id;
      e.preventDefault();
      pb.confirm(
          function (outcome) {
           if(outcome){
            $.ajax({
                      type: "GET",
                      url: "../other_controller/rollbackmasiksavings.php?ms_id="+id,
                      data: data,
                      contentType: false,
                      processData: false,
                      beforeSend: function(){
                      ...
                      success: function(data) {
                       pb.clear();
                        pb.error('<i class="fa fa-trash fa-lg" aria-hidden="true"></i>   Restored Successfully');
                        location.reload();
                      }
                  });
                }
