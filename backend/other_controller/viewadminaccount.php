 ...
 <?php
  ...
  $statement->execute(['r_name' => 'admin']);
  ...
  if ($total_row > 0) {
    foreach ($result as $a) {
      $output .= '
                        ...
                        <td>' . $a['a_address'] . '</td>';
      $output .= '<td>';
      if ($per->permit('p_editadminaccount', $pdo)) {
        $output .= '<a href="editadminaccount?a_id=' . $a['a_id'] . '"><button class="btn btn-sm btn-outline-primary update">
                        <i class="fa fa-edit"></i>
                        </button></a>';
      }
      if ($per->permit('p_deleteadminaccount', $pdo)) {
        $output .= '<button class="btn btn-sm btn-danger delete" id="' . $a['a_id'] . '">
