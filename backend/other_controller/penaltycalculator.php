...
function deadline($d1)
{
    $loan_date = date_create($d1);
    $today = date_create(date("Y-m-d"));
    $diff = date_diff($loan_date, $today);
    $monthuptonow =  $diff->format("%m"); //2 month 
    $aftermdate = $diff->format("%d"); //17 days


    if ($aftermdate > 2) { //checking if days obtained from 12 line is greater than 2 
        $new_deadline = $monthuptonow + 1;
    } else {
        $new_deadline = $monthuptonow;
    }
    $n = date('Y-m-d', strtotime($d1 . ' + ' . $new_deadline . ' months' . '2 days'));
    return $n;
}
