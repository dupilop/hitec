...
if ($_SESSION['access_level'] == 'superadmin') {
    ...
    if (isset($_POST['sdate']) && $_POST['sdate'] != '') {
        ...
        INNER JOIN customers c ON ms.c_id=c.c_id WHERE ms.c_id=:cid && ms_dateupload LIKE :sdate
        ");
        $asd->execute(['cid' => $_POST['id'], 'sdate' => '%' . $_POST['sdate'] . '%']);
        $rc1 = $asd->rowCount();
        //show loans
        $asd2 = $pdo->prepare("SELECT * FROM loans l
INNER JOIN customers c ON l.l_c_id=c.c_id 
INNER JOIN admins a ON a.a_id=l.l_uploadedby WHERE l.l_c_id=:cid && l.l_status='unpaid' && l_upload_date_time LIKE :sdate
");
        $asd2->execute(['cid' => $_POST['id'], 'sdate' => $_POST['sdate'] . '%']);
        $rc2 = $asd2->rowCount();
        //show loan transaction
        $asd3 = $pdo->prepare("SELECT * FROM loan_transactions lt
INNER JOIN customers c ON lt.c_id=c.c_id WHERE lt.c_id=:cid && lt_uploaddate LIKE :sdate
");
        $asd3->execute(['cid' => $_POST['id'], 'sdate' => $_POST['sdate'] . '%']);
        $rc3 = $asd3->rowCount();
        //show loans settlement
        $asd4 = $pdo->prepare("SELECT * FROM loans l
INNER JOIN customers c ON l.l_c_id=c.c_id  WHERE l.l_c_id=:cid && l.l_status='paid' && l_upload_date_time LIKE :sdate
");
        $asd4->execute(['cid' => $_POST['id'], 'sdate' => $_POST['sdate'] . '%']);
        $rc4 = $asd4->rowCount();
        //show customers
        ...
        $asd = $pdo->prepare("SELECT * FROM masiksavings ms
                    ...
                    WHERE ms.c_id=:cid
                    ");
        $asd->execute(['cid' => $_POST['id']]);
        $rc1 = $asd->rowCount();
        //show loans
        $asd2 = $pdo->prepare("SELECT * FROM loans l
    INNER JOIN customers c ON l.l_c_id=c.c_id 
    WHERE l.l_c_id=:cid && l.l_status='unpaid'
    ");
        $asd2->execute(['cid' => $_POST['id']]);
        $rc2 = $asd2->rowCount();
        //show loan transaction
        $asd3 = $pdo->prepare("SELECT * FROM loan_transactions lt
    ...
    WHERE lt.c_id=:cid
    ");
        $asd3->execute(['cid' => $_POST['id']]);
        $rc3 = $asd3->rowCount();
        //show loans settlement
        $asd4 = $pdo->prepare("SELECT * FROM loans l
    INNER JOIN customers c ON l.l_c_id=c.c_id 
   WHERE l.l_c_id=:cid && l.l_status='paid'
    ");
        $asd4->execute(['cid' => $_POST['id']]);
        $rc4 = $asd4->rowCount();
    }
} else {
