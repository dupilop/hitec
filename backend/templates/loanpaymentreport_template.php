    ...
    var table2 = $('#example').DataTable({
      ...
      buttons: [{
          ...
          customize: function(win) {
            ...
            $(win.document.body).find('#pdate').prepend('<div style="text-align:center;font-size:15px;color:black;"><b>PAYMENTS MADE ON : ' + mon + yrs + date + comdate + '</b></div>');
...
if ($_SESSION['access_level'] == 'superadmin') {
  $asd = $pdo->prepare("SELECT * FROM loan_transactions lt
...
<tbody>
  ...
  foreach ($asd as $a) {
    ...
    $dateObj   = DateTime::createFromFormat('!m', $a['lt_month']);
    $monthName = $dateObj->format('F');
    echo '<td>' . $monthName . '</td>';
    echo '<td>' . $a['lt_year'] . '</td>';
    echo '<td>' . $date . '</td>';
