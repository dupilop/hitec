...
<?php
  $rr = $pdo->prepare("SELECT * FROM notifications WHERE n_status='new' && n_visibility='yes' && (n_receiver='all' || n_receiver='staff') ORDER BY n_id DESC");
  $rr->execute();
  $result = $rr->fetchAll();
...
?>
                  ...
                  <ul class="dropdown-menu list-unstyled msg_list" role="menu" aria-labelledby="navbarDropdown1">
                   <?php 
                    $ss = $pdo->prepare("SELECT * FROM notifications WHERE n_receiver='all' || n_receiver='staff' ORDER BY n_id DESC");
                    $ss->execute();
                    $rc = $ss->rowCount();
                    if($rc>0){
